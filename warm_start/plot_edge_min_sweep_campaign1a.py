#!/usr/bin/env python3
"""Sweep edge_min for greedy optima on the campaign1a RF surrogate and plot."""
...
CSV_PATH = REPO / "interactive_testing" / "campaign1a.csv"
...
CORNERS = ("FAPbI3", "MAPbI3", "MAPbBr3")
OUT = REPO / "warm_start" / "optima_finder_campaign1a_rf_edge_sweep"


def build_campaign1a_rf_objective(
    csv_path: Path = CSV_PATH,
) -> tuple[object, dict]:
    df = pd.read_csv(csv_path)
    df = df.dropna(subset=COMP_COLS + [OBJECTIVE_COL])
    ...
    meta = {
        ...
        "composition_columns": list(COMP_COLS),
        "objective_column": OBJECTIVE_COL,
        "n_train": int(X.shape[0]),
        "rf_n_estimators": RF_N_ESTIMATORS,
        "rf_seed": RF_SEED,
        "objective": "RF(campaign1a)",
        "maximize": True,
    }
...
def run_one(
    ...
    *,
    ...
    out_json = out_dir / "campaign1a_optima.json"
...
def main() -> int:
    ...
    print(
        f"campaign1a RF edge_min sweep: n_optima={N_OPTIMA} seed={SEED} "
        f"design={DEFAULT_DESIGN} n_design={n_design} → {OUT}"
    )
    print(f"Training RF on {CSV_PATH.relative_to(REPO)} …")
    objective, meta = build_campaign1a_rf_objective()
    ...
    summary: dict = {
        ...
        "edge_mins": EDGE_MINS,
        **{k: meta[k] for k in (
            "csv_path", "composition_columns", "objective_column",
    ...
    fig.suptitle(
        f"campaign1a RF edge_min sweep (n={N_OPTIMA}, Sobol={n_design}, seed={SEED})",
        fontsize=11,
    )
    fig.tight_layout()
    out_png = OUT / "campaign1a_edge_sweep.png"
    ...
    fig.suptitle("campaign1a RF edge_min sweep metrics", fontsize=11)
    fig.tight_layout()
