...
**Ensemble settings** section so the view's own controls stay uncluttered.  Each
optional feature has an on/off checkbox; unchecking it passes the disabling
value (count/amplitude 0) without losing the slider position.  There is
intentionally no save/load of defaults.
...
def build_app():
    ...
    from dash import (  # noqa: F811
        Dash, Input, Output, State, callback, dcc, html,
    )
    ...
    def toggle_grid_controls(dim_sel):
        ...
        # The two views are lattices of different kinds, so each carries its own
        # resolution slider and only the live one is on screen.
