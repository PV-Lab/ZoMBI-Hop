...
Landscape (``--dataset``, default ``newRF``):
  • ``newRF`` — a FIXED Random-Forest surrogate trained on live measurements from a
    ...
    surrogate headlessly — no GUI picker, safe on HPC.
...
Both are overridable with ``--arb-hparams`` / ``--tuned-hparams`` (any file
...
DEFAULT_TUNED_HPARAMS = "optimize/runs/mobo_ensemble_3d_job17560178/trial_13/trial.json"
...
def main() -> None:
    ...
    parser.add_argument("--dataset", default="newRF", choices=["newRF", "ensemble"],
                        help="landscape: fixed RF surrogate from a DB (newRF, default) "
    ...
    # newRF is a single fixed surrogate shared by every seed; ensemble re-randomizes
    # the landscape per seed. ``landscape_for_seed`` hides that difference.
    ...
    summary = {
        ...
        "dim": 3 if args.dataset == "newRF" else args.dim,
        "n_seeds": args.n_seeds,
