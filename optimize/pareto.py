...
def collect_trials(
    ...
    *only_dims*: if set, include every run whose name contains any of these
    dimension substrings (e.g. ``_4d_`` from the ``--only 4d`` shorthand, which
    matches both ``mobo_4d_*`` and ``mobo_ensemble_4d_*``).
    """
    has_filter = only_runs or only_trials or only_dims
    records: list[dict] = []
    ...
    for path in progress_paths:
        ...
        if has_filter:
            matches_dim = any(tok in run_name for tok in (only_dims or set()))
...
def main() -> None:
    ...
    only_runs, only_trials, only_dims = (
