...
_scaling = _load_module("_metric_scaling", "src/utils/scaling.py")
_simplex = _load_module("_metric_simplex", "src/utils/simplex.py")
dimension_line_scale = _scaling.dimension_line_scale
dimension_radius_scale = _scaling.dimension_radius_scale
...
def unmatched_penalty(dim: int) -> float:
    """Per-miss penalty scaled with ``(d−1)/2`` (matches ``scaled_n_optima``)."""
...
def dup_threshold_ilr(dim: int) -> float:
    """Duplicate-sample radius in ILR L2 (half input-noise scale × Group 1)."""
    return (NOISE_LEVEL_ILR / 2.0) * dimension_radius_scale(dim)
