...
Forwards to the unified evaluate CLI.  ``--time-limit-hours`` is accepted here
and converted to ``--time-limit-min`` for evaluate.py.
"""
...
def main() -> None:
    ...
    parser.add_argument("--time-limit-hours", type=float, default=None,
                        help="Converted to --time-limit-min for evaluate.py.")
    ...
    if args.config:
        ...
        parser.error("Provide --config or --oracle.")

    cmd.extend(["--dim", str(args.dim), "--layout", args.layout, "--seed", str(args.seed)])
    if args.time_limit_min is not None:
        cmd.extend(["--time-limit-min", str(args.time_limit_min)])
    elif args.time_limit_hours is not None:
        cmd.extend(["--time-limit-min", str(args.time_limit_hours * 60.0)])
    if args.out_dir:
