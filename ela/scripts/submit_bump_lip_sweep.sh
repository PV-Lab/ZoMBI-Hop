#!/usr/bin/env bash
# Feed the bump (RBF primitive) × Lipschitz-variant sweep under a CPU concurrency cap.
#
# This is the paper-similar bump analogue of the m8lip0 sweep: every member uses
# upweighted localized ILR Gaussian bumps (paper_ga=false + allow_rbf=true, depth=12,
# mut=0.35). Muñoz-8+Lip core, α=0, and paper GA hyperparameters. Each member adds a
# different spatial-Lipschitz variant on top of the shared base.
#
...
#
# Usage (from repo root on ORCD login node — leave running / use tmux):
...
#   PILOT_SUBMIT_MODE=slurm-queue bash ela/scripts/submit_bump_lip_sweep.sh
#
set -euo pipefail
...
SBATCHES=(
  run_pilot_3d_bump_base.sbatch
  run_pilot_3d_bump_interior.sbatch
  run_pilot_3d_bump_pct.sbatch
  run_pilot_3d_bump_cv.sbatch
  run_pilot_3d_bump_tile.sbatch
  run_pilot_3d_spatial_modes.sbatch
)
...
echo
echo "All ${n} sweep jobs submitted. Log: ela/scripts/logs/bump_lip_sweep_submitted.txt"
echo "squeue -u \$USER"
