#!/usr/bin/env bash
# Set C NBC-12 RBF sweep: no RBF | mild RBF | upweighted RBF
#
...
SBATCHES=(
  run_pilot_3d_set_c_nbc12_norbf.sbatch
  run_pilot_3d_set_c_nbc12_rbf.sbatch
  run_pilot_3d_set_c_nbc12_rbf_up.sbatch
)
...
_submit_one() {
  ...
  echo "${job_id}  ${script}" >> ela/scripts/logs/set_c_nbc12_rbf_sweep_submitted.txt
}

if [[ "${MODE}" == "slurm-queue" ]]; then
  echo "Mode=slurm-queue: submitting all ${#SBATCHES[@]} jobs."
  : > ela/scripts/logs/set_c_nbc12_rbf_sweep_submitted.txt
...
echo "All ${n} sweep jobs submitted. Log: ela/scripts/logs/set_c_nbc12_rbf_sweep_submitted.txt"
