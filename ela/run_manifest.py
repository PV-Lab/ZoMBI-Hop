...
def build_start_manifest(
    *,
    ...
    cfg_log = pilot_config.to_log_dict() if hasattr(pilot_config, "to_log_dict") else pilot_config
    return {
        "schema": "ela_s1_run_manifest_v2",
        ...
        "mode": cfg_log.get("mode_label", cfg_log.get("mode")),
        "pilot_config": {
            "source": str(config_path.resolve()),
            "resolved": str(resolved_config_path.resolve()),
            "name": cfg_log.get("name"),
            "parameters": cfg_log,
        },
        "munoz_8_fitness": cfg_log["fitness"]["munoz_8_fitness"],
        "linear_calibration": cfg_log["fitness"]["linear_calibration"],
        "require_subspace_rmse": cfg_log["fitness"]["require_subspace_rmse"],
        "paths": {
            "run_dir": str(run_dir.resolve()),
            "db": cfg_log["data"]["db"],
            "target": cfg_log["data"]["target"],
            "pilot_log": str((run_dir / "pilot.log").resolve()),
            "config": str((run_dir / "config.json").resolve()),
            "pilot_config_source": str((run_dir / "pilot_config.source.json").resolve()),
            "pilot_config_resolved": str(resolved_config_path.resolve()),
        ...
        "hyperparameters": {
            ...
            "alpha_subspace": cfg_log["fitness"]["alpha_subspace"],
            "beta_complexity": cfg_log["fitness"]["beta_complexity"],
            "tier1_gamma": cfg_log["fitness"]["tier1_gamma"],
            ...
            "landscape_grid_n": cfg_log["viz"]["landscape_grid_n"],
            "grid_n": cfg_log["viz"]["grid_n"],
            "early_reject_mult": cfg_log["ga"]["early_reject_subspace_mult"],
            "quick": bool(cfg_log["runtime"]["quick"]),
            "no_landscape_viz": not cfg_log["viz"]["landscape_viz"],
            "no_viz": not cfg_log["viz"]["post_viz"],
        },
        "environment": {
